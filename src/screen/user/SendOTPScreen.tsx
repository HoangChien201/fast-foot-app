...
const SendOTPScreen: React.FC<SendOTPScreenProp> = ({ route, navigation }) => {
  ...
  return (
    <View style={styles.container}>
      <View>
        <View>
          <Text style={[styles.text, styles.heading]}>Xác thực OTP</Text>
          <Text style={[styles.text, styles.description]}>Nhập mã OTP đã gửi tới {email}</Text>
        </View>
        ...
        </View>
        {
          duration > 0 ? <Text style={[styles.text, styles.duration]}>Resend code in <Text style={{ color: 'red' }}>{duration}s</Text></Text>
            :
            <Text style={styles.text}>Hết giờ</Text>
